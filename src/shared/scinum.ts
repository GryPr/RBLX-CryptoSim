...
class SciNumKit {
  ...
  subtract(minuend: SciNum, subtrahend: SciNum): SciNum {
    ...
    if (m.Exponent > s.Exponent) {
      if (m.Exponent - s.Exponent > 10) {
        ...
        difference.Base =
          m.Base - s.Base / math.pow(10, m.Exponent - s.Exponent);
        difference.Exponent = m.Exponent;
      }
      ...
      if (s.Exponent - m.Exponent > 10) {
        ...
        difference.Base =
          -s.Base + m.Base / math.pow(10, s.Exponent - m.Exponent);
        difference.Exponent = s.Exponent;
      }
    } else if ((s.Exponent = m.Exponent)) {
  ...
  truncate(n: SciNum): SciNum {
    ...
    if (
      this.order(n.Base) <= minimumToSci &&
      n.Exponent <= minimumToSci - magnitude
    ) {
      return n;
    }
    if (this.order(n.Base) > magnitude) {
      truncated.Base =
        truncated.Base / math.pow(10, this.order(n.Base) - magnitude);
  ...
  isSciNum(obj: any): obj is SciNum {
    return (
      (obj as SciNum).Base !== undefined &&
      (obj as SciNum).Exponent !== undefined
    );
  }
}
